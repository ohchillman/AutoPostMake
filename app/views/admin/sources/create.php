<!-- Представление для создания источника новостей -->
<div class="card">
    <div class="card-header">
        <h5 class="card-title">Добавление источника новостей</h5>
    </div>
    <div class="card-body">
        <form method="POST" action="/admin/sources.php?action=store">
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="url" class="form-control" id="url" name="url" value="<?php echo isset($data['url']) ? htmlspecialchars($data['url']) : ''; ?>" required>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="parser_type" name="parser_type" required>
                    ...
                    <option value="rss" <?php echo (isset($data['parser_type']) && $data['parser_type'] === 'rss') ? 'selected' : ''; ?>>RSS</option>
                    <option value="html" <?php echo (isset($data['parser_type']) && $data['parser_type'] === 'html') ? 'selected' : ''; ?>>HTML</option>
                </select>
                <div class="form-text">Выберите тип парсера в зависимости от формата источника</div>
            </div>
            
            <div class="mb-3" id="selector_group" style="display: none;">
                <label for="selector" class="form-label">CSS селектор</label>
                <input type="text" class="form-control" id="selector" name="selector" value="<?php echo isset($data['selector']) ? htmlspecialchars($data['selector']) : ''; ?>">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const parserType = document.getElementById('parser_type');
        if (parserType.value === 'html') {
            document.getElementById('selector_group').style.display = 'block';
        }
    });
</script>
